...
def split_pretrain_data(token_labels, token_embeddings, token_node_ids):
    ...
    return (
        train_node_token_embeddings,
        train_node_token_labels,
        eval_node_token_embeddings,
        eval_node_token_labels,
    )
...
class PretrainData(data_.Dataset):
    ...
    def __getitem__(self, idx):
        return (
            self.node_ids[idx],
            self.neighbor[idx],
            self.node_token_embeddings[idx],
            self.node_token_labels[idx],
            self.node_token_weight[idx],
        )
...
def pretrain_graph_adapter(args):
    ...
    (
        ...
        eval_node_token_embeddings,
        eval_node_token_labels,
    ) = split_pretrain_data(token_labels, token_embeddings, token_node_ids)
    ...
    train_data = PretrainData(
        list(range(data.x.shape[0])),
        data.edge_index,
        train_node_token_embeddings,
        train_node_token_labels,
        train_node_token_weight,
    )
    eval_data = PretrainData(
        list(range(data.x.shape[0])),
        data.edge_index,
        eval_node_token_embeddings,
        eval_node_token_labels,
    ...
    model = GraphAdapter(
        llm_shape=data.x.shape[1],
        hiddensize_gnn=hiddensize_gnn,
        hiddensize_fusion=hiddensize_fusion,
