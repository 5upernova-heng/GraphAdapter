...
def get_prompt_embedding(model, tokenizer, x, template_l, template_r, device, args=None):
    ...
    feature_ls_ids = []
    for f in feature_ls:
        feature_ls_ids.append(tokenizer(template_l + f + template_r, padding=True, truncation=True)["input_ids"])
    nodedata_ = RawTextData(feature_ls_ids, list(range(len(feature_ls))))
    node_data_loader = DataLoader(
        nodedata_,
        batch_size=args.batch_size,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--plm_path",
        type=str,
        default="/data/pretrain_models/llama-2-13b-hf",
        help="path of llama 2",
    )
    ...
    parser.add_argument(
        ...
        help="path of saving pretrain data",
    )
    parser.add_argument(
        "--prompt_save_path",
        type=str,
        default="./prompt_embedding/",
        help="path of saving prompt embedding",
    )
    parser.add_argument(
        "--type",
        type=str,
        ...
        choices=["pretrain", "prompt", "all", "convert"],
    )
    ...
    if (args.type == "pretrain") or (args.type == "all"):
        ...
        (
            ...
            token_node_ids_path,
            token_label_path,
        ) = build_pretrain_data_by_tables(model, tokenizer, x_text, save_path, template_l, args.device, args)
